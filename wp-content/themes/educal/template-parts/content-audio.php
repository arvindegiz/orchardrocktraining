 ...
 */
    ...
    $educal_audio_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;
...
?>
    <article id="post-<?php the_ID();?>" <?php post_class( 'blog__item white-bg mb-50 transition-3 format-audio' );?>>
        <?php if ( !empty( $educal_audio_url ) ): ?>
            <div class="postbox__audio embed-responsive embed-responsive-16by9">
                <?php echo wp_oembed_get( $educal_audio_url ); ?>
            </div>
        <?php endif;?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'blog__item white-bg mb-50 transition-3 fix format-audio' );?> data-wow-delay=".3s">
        <?php if ( !empty( $educal_audio_url ) ): ?>
